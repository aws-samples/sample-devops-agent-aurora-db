...
| **Database secrets are not auto-rotated** — the Aurora master secret and the `mcp_app` app-role secret have no rotation schedule; only the public-facing MCP **API key** rotates automatically. | The API key is the credential exposed to the internet, so it is the one that rotates. The DB secrets are reached only over the RDS Data API (master used solely by operator scripts, app secret by a read-only role), and rotating them properly would require a VPC-attached RDS rotation Lambda with network access to Aurora — disproportionate for a demo, and a rotation schedule without that network path would silently fail. | Enable `AWS::SecretsManager::RotationSchedule` with the AWS-managed RDS PostgreSQL rotation Lambda (`HostedRotationLambda`) for both DB secrets, giving the rotation function VPC access (or PrivateLink) to the cluster. Pair with the in-VPC hardening in the row below. |
| **Lambdas run outside a VPC** — the MCP handler, authorizer, and rotation functions have no `VpcConfig`. | The MCP Lambda reaches Aurora only through the **RDS Data API** (a public AWS endpoint over TLS + SigV4), not a private DB socket, so there is no in-VPC data path to protect; adding a VPC would only force PrivateLink endpoints for `rds-data`, `secretsmanager`, and `rds` to restore existing connectivity. | For a locked-down deployment, attach the functions to private subnets with a dedicated security group **and** add interface VPC endpoints (PrivateLink) for `rds-data`, `secretsmanager`, `rds`, and `kms` so all traffic stays on the AWS network. The data-plane risk is already bounded here by the read-only role, `SET TRANSACTION READ ONLY`, and cluster-scoped IAM. |
